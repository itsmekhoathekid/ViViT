...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0., attention = Attention):
        ...
        self.ff = FeedForwardBlock(dim, mlp_dim, dropout = dropout)
        self.residual_list = nn.ModuleList([
            ResidualConnection(dim, dropout = dropout) for _ in range(depth)
        ])

    def forward(self, x):
        for res in self.residual_list:
            x = res(x, self.attention)
            x = res(x, self.ff)
        return x
